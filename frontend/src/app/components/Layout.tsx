    ...
    <div className="h-screen flex flex-col">
      ...
      <header className="h-16 border-b border-[--border-light] bg-white flex items-center justify-between px-6 shadow-sm">
        <Link to="/" className="flex items-center gap-2">
          <Factory className="size-6" style={{ color: 'var(--primary-500)' }} />
          <span className="font-semibold text-lg" style={{ color: 'var(--text-heading)' }}>Factory System</span>
        </Link>
        ...
        <div className="flex items-center gap-4">
          <Button variant="ghost" size="icon" className="relative" onClick={() => navigate('/notifications')}>
            ...
            {unreadCount > 0 && (
              <Badge style={{ backgroundColor: 'var(--accent-500)', color: 'var(--text-on-dark)' }} className="absolute -top-1 -right-1 size-5 rounded-full p-0 flex items-center justify-center text-xs border-0">
                {unreadCount}
              </Badge>
            )}
          </Button>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuLabel>
                <div className="flex flex-col">
                  <span>{user.name}</span>
                  <span className="text-xs font-normal" style={{ color: 'var(--text-muted)' }}>{user.email}</span>
      ...
      <div className="flex-1 flex overflow-hidden">
        {/* Left Sidebar */}
        <aside className="w-64 border-r" style={{ backgroundColor: 'var(--bg-sidebar)', borderColor: 'var(--sidebar-border)' }}>
                  ...
                  <div key={item.path}>
                    {hasChildren ? (
                      <button
                        ...
                        onMouseEnter={(e) => {
                          if (!isExpanded) e.currentTarget.style.backgroundColor = 'var(--sidebar-hover)';
                        }}
                        onMouseLeave={(e) => {
